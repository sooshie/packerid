...
#
# 2014-12-03 - incremented to 1.5, moved over to Argument Parser and created terse output - sooshie@gmail.com
...
for file in vars(args)['files']:
    ...
    if args.terse:
        ...
        for section in pe.sections:
              print "\t%s %s %s %s" %(section.Name, hex(section.VirtualAddress), hex(section.Misc_VirtualSize), section.SizeOfRawData )
        print "Imports"
        try:
        ...
        try:
            for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols:
                print "%s %s" %(exp.name, exp.ordinal)
        except AttributeError as e:
            print "\tNone"
